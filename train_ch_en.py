"""
中英翻译版训练脚本。对应原版 train.py，训练循环一个字没改——
改的只有 import、字典构建的参数、以及输出路径（原版硬编码 saved_chkpt/ 和 results/，
会覆盖捷克语流程的产物）。
...
################################################################################
# 建词表：读 CSV -> 限流 max_pairs -> 按 min_count 裁掉低频词（归入 UNK）
# 加 save=True（默认）落盘到 saved_chkpt_ch_en/，供 translate_ch_en.py 复用同一张表
...
################################################################################
# 只忽略 PAD，不忽略 UNK——UNK 是词表里合法的输出目标（低频词都映到它），
# 屏蔽掉会让模型没有动力在这些位置输出任何东西
...
def train(model, train_dataloader, clip, epoch_num,  label_smoothening=False):
    model.train()
    epoch_loss = 0
    # 直接迭代 tqdm 包装的 DataLoader,进度条自动推进
    with tqdm(train_dataloader, unit="batches", desc=f"Epoch {epoch_num+1}") as tepoch:
...
def run(total_epoch, best_loss, best_epoch):
    ...
    for step in range(total_epoch):
        ...
        # DataLoader 每次迭代自动重新打乱,无需手动建 batch
        ...
        if valid_loss <= best_loss:
            ...
            torch.save(model.state_dict(), os.path.join(utils.saved_chkpt, 'best_model.pt'))
            best_epoch = step
        # else:
        # 	early_stop_counter+=1

        f = open(os.path.join(utils.results, 'train_loss.txt'), 'w')
        f.write(str(train_losses))
        f.close()

        f = open(os.path.join(utils.results, 'bleu.txt'), 'w')
        f.write(str(bleus))
        f.close()

        f = open(os.path.join(utils.results, 'valid_loss.txt'), 'w')
