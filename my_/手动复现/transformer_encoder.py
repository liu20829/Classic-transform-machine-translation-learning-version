...
class positional(nn.Module):
    def __init__(self,model_dim:int,len_q:int,device:str):
        ...
        pass
    def forward(self):
        pos=torch.arange(0,self.len_q,dtype=torch.float,device=self.device).reshape(-1,1)

        idx=torch.arange(0,self.model_dim,2,device=self.device,dtype=torch.float)

        angles=pos/(10000**(idx/self.model_dim))

        pe=torch.zeros(self.len_q,self.model_dim,device=self.device)
...
class embedding_positional(nn.Module):
    ...
    def forward(self,x):
        ...
        embedding_x_pos=self.dropout((embedding_x*self.coefficient+self.positional()))
...
class transformer_encoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(encode_num):
            self.encode_num_layer.append(transformer_encoder_layer(model_dim,num_heads,dropout,feedforward_dim,device))
